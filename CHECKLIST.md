...
- [x] **H8 closed, the one hitch left on the lap**: `attachForWarmup` bailed when `model.parent` was set — but a dormant dino keeps its model on its object and detaches the OBJECT (M24), so `model.parent` proves nothing and every species at load was skipped. It walks to the root and compares with the scene now. The wood line's 144 ms depth compile is gone; the whole lap is 9-25 ms worst-frame with zero programs compiled
